...
@section('content')
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="card">
                <div class="card-body">
                    {!! Form::open(['method' => 'post', 'route' => [Auth::user()->role.'.pengguna.store'], 'enctype' => 'multipart/form-data']) !!}
                        ...
                        <div class="form-group">
                            ...
                            <select name="role" class="form-control">
                                ...
                                @foreach($role as $key => $value)
                                    <option value="{{ $value }}" @if($value==old('role')) selected @endif>{{ strtoupper($value) }}</option>
                                @endforeach
                            </select>
                            <i class="text-danger">{{ $errors->first('email') }}</i>
                        </div>
